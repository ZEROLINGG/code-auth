 ...
 * 在 Hono 中间件或入口处初始化
 */
export async function initLog(db: D1Database, queue: Queue): Promise<void> {
...
class Logger {
    ...
    constructor(private env: LogEnv) {
        this.db = env.AUTH_DB;
     ...
     * 发送日志到队列（带降级处理）
     */
    private async enqueue(level: LogLevel, tag: string, message: string): Promise<void> {
        // Console 输出（开发调试）
     ...
     * 直接写入 D1（降级方案）
     */
    private async writeToDB(level: LogLevel, tag: string, message: string, timestamp: number): Promise<void> {
        try {
            await this.init();
            await this.db
     ...
     * 输出到控制台
     */
    private logToConsole(level: LogLevel, tag: string, message: string): void {
    ...
    async d(tag: string, msg: string): Promise<void> {
        try {
            await this.init();
    ...
    async i(tag: string, msg: string): Promise<void> {
        try {
            await this.init();
    ...
    async w(tag: string, msg: string): Promise<void> {
        try {
            await this.init();
    ...
    async e(tag: string, msg: string): Promise<void> {
        try {
            await this.init();
    ...
    async query(levels: LogLevel[] = [], limit = 100): Promise<LogEntry[]> {
        try {
            await this.init();
            ...
            const { results } = await this.db.prepare(sql).bind(...params).all<LogEntry>();
            return results || [];
        } catch (error) {
...
export class log {
    ...
    static e(tag: string, msg: string | Error) {
        if (!logger) {
            console.error(`[${tag}] ${msg}`);
            if (msg instanceof Error) console.error(msg.stack);
            return;
        }
    ...
    static async query(levels: LogLevel[] = [], limit = 100): Promise<LogEntry[]> {
        if (!logger) {
 ...
 */
export async function handleLogQueue(
    ...
    try {
        ...
        const stmt = env.AUTH_DB.prepare(
            'INSERT INTO logs (level, tag, message, created_at) VALUES (?, ?, ?, ?)'
        );
