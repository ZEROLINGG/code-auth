...
// 缓存变量
    ...
    static async getAesKey(serverKey: string): Promise<ArrayBuffer> {
        ...
        try {
            // 使用 SHA-256 派生 32 字节密钥（与 Rust 端一致）
            const keyData = new TextEncoder().encode(serverKey);
            const hash = await crypto.subtle.digest('SHA-256', keyData);

            // 导出并缓存密钥
            this.cachedAesKeyBuffer = hash;
    ...
    /**
     * 使用 AES-GCM 加密数据
     * @param data - 待加密的数据（string 或 Uint8Array）
     * @param keyBytes - AES 密钥的 ArrayBuffer
     * @returns 包含 IV + 密文 + Tag 的 Uint8Array
     * @throws 当加密失败时抛出错误
     */
    static async encrypt_ToUint8Array(
        ...
        try {
            ...
            const iv = crypto.getRandomValues(
                new Uint8Array(this.CRYPTO_CONFIG.IV_LENGTH)
            );
     ...
     */
    static async encrypt_ToBase64(plaintext: string, keyBytes: ArrayBuffer): Promise<string> {
        const encrypted = await AES.encrypt_ToUint8Array(plaintext, keyBytes);
     ...
     */
    static async decrypt_ToUint8Array(encrypted: Uint8Array | string, keyBytes: ArrayBuffer): Promise<Uint8Array> {
        try {
            // 导入密钥
            const key = await crypto.subtle.importKey(
                'raw',
