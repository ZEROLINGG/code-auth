...
import { code_v1_generate } from "../tool/pkg";
...
export async function authSuperAdmin(c) {
...
superAdmin.post("/code/generate-by-id", async (c) => {
    try {
        ...
        // 4. 生成激活码
        const code = code_v1_generate(productKey, productId, codeValidDuration, useMaxDuration, maxUses, prebind ?? null);
        if (!code) {
...
superAdmin.post("/code/batch-generate-by-id", async (c) => {
    try {
        ...
        for (let i = 0; i < count; i++) {
            const code = code_v1_generate(productKey, productId, codeValidDuration, useMaxDuration, maxUses, prebindValue);
            if (!code) {
                return c.json({
                    success: false,
