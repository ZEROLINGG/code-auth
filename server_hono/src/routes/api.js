...
import { rsa_check_pubkey, base91_decode_to_bytes, base91_encode_from_bytes, rsa2048_decrypt_to_bytes, rsa2048_encrypt_from_bytes, aes256gcm_decrypt_to_str, code_v1_pre_parse, code_v1_verify_parse, blake3_digest_from_str, aes256gcm_encrypt_from_str, hkdf_sha256_derive_from_bytes, base85_encode_from_bytes, xchacha20poly1305_encrypt_from_str, base85_decode_to_bytes, xchacha20poly1305_decrypt_to_str } from "../tool/pkg/";
...
function Ok(c, data, message = 'ok') {
    return c.json({ success: true, code: 200, message, data });
}
function Err(c, message = 'error', code = 400) {
...
api.post("/auth/reg/code/v1", async (c) => {
    ...
    const code_used_json = await kv.get(code_used_key, "json");
