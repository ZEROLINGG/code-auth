...
import {AuthResult, Code} from "../tool/code";
...
function splitUint8Array(data: Uint8Array, maxChunkSize: number): Uint8Array[] {
...
function ok(c: Context, data?: Record<string, unknown>): Response {
    return c.json({ status: "ok", message: "", code: "200", ...data });
}

/** 统一错误响应 */
function error(c: Context, error: ApiError | string, statusCode = 400): Response {
...
/** RSA 解密封装 */
async function decrypt(data: string, privateKey: string): Promise<string> {
    try {
        return await RSA.decrypt_ToString(data, privateKey);
    } catch (e) {
...
api.use("*", async (c, next) => {
    try {
        ...
        if (e instanceof ApiError) {
            return error(c, e, e.statusCode);
        }
        return error(c, ApiError.serverError(), 500);
    }
});
...
api.post("/auth/reg/code", async (c) => {
    ...
    const { serverPriPem, clientPubPem } = await getSessionContext(kv, body.client_uuid);
        ...
        // [success: boolean, uuid: string, code_expirationTime: number, use_expirationTime: number, bindingHash: string, remaining: number]
        const authResult: AuthResult = await Code.auth(
            kv,
            c.env.SERVER_KEY,
            code,
            productId,
        ...
        const jsonData = splitUint8Array(encoder.encode(JSON.stringify(authResult)), 90);

        let data: string[] = [];
        for (const d of jsonData) {
            data.push(await RSA.encrypt(d, clientPubPem))
        }
...
/**
 * [POST] /auth/again/reg/code - 后期认证接口
 */
api.post("/auth/again/reg/code", async (c) => {
    ...
    const bindingHash = await decrypt(body.data_b, serverPriPem);

    // 执行重新认证
    const authResult: AuthResult = await Code.authAgain(
        kv,
        activationUuid,
        bindingHash
    );
